...
def GetConfig(user_config):
  ...
  if 'vm_groups' in config:
    default_group = config['vm_groups'].get('default')
    if default_group and default_group.get('vm_count', 0) == 0:
      config['vm_groups'].pop('default', None)
  else:
    config['vm_groups'] = {'default': {'vm_count': max(FLAGS.num_vms, 2)}}
  return config
...
def PrepareVM(vm):
  ...
  """
  vm.AllowPort(12865, 60999)
  vm.RemoteCommand('sudo iptables -P INPUT ACCEPT && sudo iptables -P OUTPUT ACCEPT && sudo iptables -F')
  vm.RemoteCommand('./netserver -p 12865 -L 0.0.0.0')
...
def RunNetperf(vm, benchmark_name, servers, result):
  ...
  for server in servers:
    if vm != server:
      cmd += (
          './netperf -t '
          '{benchmark_name} -H {server_ip} -i {iterations} -r 1,1 '
          '{cmd_suffix} & '
      ).format(
